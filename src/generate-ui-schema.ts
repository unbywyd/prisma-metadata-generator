        ...
        if (!displayField.displayExpression) {
            switch (field.type) {
                ...
                case 'Number':
                    ...
                    break;
                case 'Boolean':
                    displayField.displayExpression = `${field.name} ? 'Yes' : 'No'`;
                    break;
                case 'Json':
                    displayField.displayExpression = `JSON.stringify(model.${field.name})`;
                    break;
                case 'Enum':
                ...
                case 'Relation':
                    if (field.isList) {
                        displayField.displayExpression = `model.${field.name}.map(item => item.name).join(', ')`;
                    } else {
                        displayField.displayExpression = `model.${field.name}?.name || ''`;
                    }
                    break;
                default:
                    displayField.displayExpression = `model.${field.name}`;
